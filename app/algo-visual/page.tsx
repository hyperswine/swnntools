...
class FlowChartGenerator {
  ...
  generateFlowChart(definitions: FunctionDefinition[]): { nodes: Node[], edges: Edge[] } {
    ...
    definitions.forEach((def, defIndex) => {
      this.generateFunctionFlow(def, defIndex * 300);
    });

    return { nodes: this.nodes, edges: this.edges };
  }

  private generateFunctionFlow(def: FunctionDefinition, yOffset: number) {
    ...
    // Create pattern matching branches
    def.patterns.forEach((pattern, index) => {
      ...
      this.nodes.push({
        ...
        data: { label: this.formatPattern(pattern) },
      ...
      // Generate body flow
      const bodyNodes = this.generateTermFlow(def.body, bodyId, 100 + index * 200, currentY + 80);
